...
through CI. Artwork is generated raster PNG content committed under the app
asset tree; the generator only references those stable paths and never
rewrites binary artwork.
"""
...
ASSET_DIR = ROOT / "app/assets/illustrations/journey/content-pack/v1-generated"
...
def write_assets() -> dict[str, dict[str, str]]:
    """Return stable asset paths for the pre-generated PNG artwork.

    The content pack owns 240 PNGs (30 covers, 90 checkpoints, 30 landmarks,
    30 foods and 60 destination items).  Keep generation deterministic while
    avoiding accidental overwrites of the generated artwork.
    """
    ASSET_DIR.mkdir(parents=True, exist_ok=True)
    paths: dict[str, dict[str, str]] = {}
    expected_files: set[str] = set()
    for index, (slug, *_rest) in enumerate(DESTINATIONS, start=1):
        base = f"assets/illustrations/journey/content-pack/v1-generated/{slug}"
        paths[slug] = {
            "cover": base + ".png",
            "map": base + ".png",
            "landmark": base + "-landmark.png",
            "food": base + "-food.png",
            "item_a": base + "-item-a.png",
            "item_b": base + "-item-b.png",
        }
        ...
        for checkpoint in range(1, 4):
            paths[slug][f"checkpoint_{checkpoint}"] = base + f"-checkpoint-{checkpoint}.png"
            expected_files.add(f"{slug}-checkpoint-{checkpoint}.png")
    actual_files = {path.name for path in ASSET_DIR.glob("*.png")}
    missing = sorted(expected_files - actual_files)
    extra = sorted(actual_files - expected_files)
    if missing or extra or len(actual_files) != 240:
        details = []
        if missing:
            details.append("missing=" + ",".join(missing))
        if extra:
            details.append("extra=" + ",".join(extra))
        details.append(f"count={len(actual_files)} (expected 240)")
        raise RuntimeError("content-pack artwork manifest mismatch: " + "; ".join(details))
    return paths
...
def build_migration(paths: dict[str, dict[str, str]]) -> str:
    lines = [
        ...
        "-- Generated by supabase/seed/generate_content_pack.py; artwork paths reference 240 generated PNGs.",
        "-- Do not edit generated rows by hand; update the source generator/content pack instead.",
...
def main() -> None:
    ...
    print(f"generated {MIGRATION} and {len(list(ASSET_DIR.glob('*.png')))} PNG assets")
